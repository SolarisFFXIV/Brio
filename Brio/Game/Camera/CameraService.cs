...
public unsafe class CameraService : IDisposable
{
    ...
    public CameraService(EntityManager entityManager, VirtualCameraManager virtualCameraService, CutsceneManager cutsceneManager, GPoseService gPoseService, ISigScanner scanner, IGameInteropProvider hooking)
    {
        ...
        {
            Brio.Log.Error(ex, "Failed to hook camera projection. Camera FOV overrides will be disabled.");
        }
        ...
        {
            Brio.Log.Error(ex, "Failed to hook camera collision. Camera collision overrides will be disabled.");
        }
        ...
        {
            Brio.Log.Error(ex, "Failed to hook camera update. Camera offsets will be disabled.");
        }
        ...
        {
            Brio.Log.Error(ex, "Failed to hook camera scene update. Free-cam and cutscene camera overrides will be disabled.");
        }
        ...
        {
            Brio.Log.Error(ex, "Failed to resolve camera matrix loader. Some camera overrides will be disabled.");
        }
    }

    private nint CameraUpdateDetour(BrioCamera* camera)
    {
        var result = _cameraUpdateHook?.Original(camera) ?? 0;
    ...
    {
        var exec = _cameraSceneUpdateHook?.Original(gsc) ?? 0;
    ...
    private nint CameraCollisionDetour(BrioCamera* camera, Vector3* a2, Vector3* a3, float a4, nint a5, float a6)
    {
        ...
        return _cameraCollisionHook?.Original(camera, a2, a3, a4, a5, a6) ?? 0;
    }
