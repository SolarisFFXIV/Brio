...
public static class PosingEditorCommon
{
    ...
    {
        if(ImBrio.FontIconButton("reset_defaults", FontAwesomeIcon.Undo, "Reset Defaults"))
        {
        ...
        if(posingService is not null)
        {
            ...
            if(ImBrio.ToggelFontIconButton("show_all_actor_bones", FontAwesomeIcon.Users, new(0), posingService.ShowBonesForAllVisibleActors, hoverText: "Show bones for all visible actors"))
            {
                posingService.ShowBonesForAllVisibleActors = !posingService.ShowBonesForAllVisibleActors;
            }
        }

        var tristateCheckbox = new ImBrio.TristateCheckbox();

        // Get IVCS and Other category references for filtering
        var ivcsCategoryRef = filter.AllCategories.FirstOrDefault(c => c.Id == "ivcsCategory");
        var otherCategoryRef = filter.AllCategories.FirstOrDefault(c => c.Id == "otherCategory");
        
        var ivcsAndOtherBoneIds = new HashSet<string>();
        if(ivcsCategoryRef != null)
            ivcsAndOtherBoneIds.UnionWith(ivcsCategoryRef.Bones);
        if(otherCategoryRef != null)
            ivcsAndOtherBoneIds.UnionWith(otherCategoryRef.Bones);
        ...
        foreach(var category in filter.AllCategories)
        {
            if(category.Type is BoneCategories.BoneCategoryTypes.Category)
            {
                ...
                // For IVCS and Other categories, use collapsing header
                if(category.Id is "ivcsCategory" or "otherCategory")
                {
                    if(ImGui.CollapsingHeader(category.Name, ImGuiTreeNodeFlags.None))
                    {
                        foreach(var childCategory in childCategories)
                        {
                            var isEnabled = filter.IsCategoryEnabled(childCategory);

                            if(ImGui.Checkbox(childCategory.Name, ref isEnabled))
                            {
                                if(isEnabled)
                                    filter.EnableCategory(childCategory);
                                else
                                    filter.DisableCategory(childCategory);

                                UpdateParentCategoryState(filter, childCategory);
                            }
                            
                            if(ImGui.IsItemHovered() && ImGui.IsMouseClicked(ImGuiMouseButton.Right))
                            {
                                filter.EnableOnly(childCategory);
                            }
                        }
                    }
                }
            ...
            {
                // Skip displaying IVCS and Other bones inline - they only appear under their headers
