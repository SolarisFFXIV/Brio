...
public class PoseFileInfo : AppliableActorFileInfoBase<PoseFile>
{
    ...
    public override void DrawActions(FileEntry fileEntry, bool isModal)
    {
        ...
        if(EntityManager.SelectedEntity is ActorEntity selectedActor &&
           ImGui.IsKeyDown(ImGuiKey.LeftCtrl) == false &&
           ImGui.IsKeyDown(ImGuiKey.RightCtrl) == false)
        {
            ImGui.SameLine();
            if(ImGui.Button("Apply as Expression"))
            {
                if(Load(fileEntry.FilePath) is PoseFile file)
                {
                    Apply(file, selectedActor, true);
                }
            }
        }
    }
